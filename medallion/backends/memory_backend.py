...
class Meta:
    ...
    def __init__(self, date_added, media_type, version):
        self.date_added = timestamp_to_datetime(date_added)
        self.version = timestamp_to_datetime(version)
        self.media_type = media_type

        # Extract and cache the spec version from media_type.  Expected sample
        # format for media_type:
        #     application/stix+json;version=2.1
        eq_idx = self.media_type.index("=")
        self.spec_version = self.media_type[eq_idx+1:]

        # A plain string is probably fine for equality comparisons, but has
        # the wrong order semantics.  E.g. "2.10" < "2.2" as strings, which is
        # incorrect.  So cache an integer tuple for ordering purposes.
        self.spec_version_tuple = tuple(
            int(version_part)
            for version_part in self.spec_version.split(".")
        )

    def __repr__(self):
        return 'Meta("{}", "{}", "{}")'.format(
            timestamp_to_taxii_json(self.date_added),
            self.media_type,
            timestamp_to_taxii_json(self.version)
        )
...
class MetaEncoder(json.JSONEncoder):
    ...
    def default(self, value):
        if isinstance(value, Meta):
            return {
                "date_added": timestamp_to_taxii_json(value.date_added),
...
def _make_manifests(objects):
    ...
    manifests = [
        {
            "id": obj["id"],
            "date_added": timestamp_to_taxii_json(obj["__meta"].date_added),
            "version": timestamp_to_stix_json(obj["__meta"].version),
...
class MemoryBackend(Backend):
    ...
    def _pop_old_statuses(self):
        ...
        for ar in api_roots:
            ...
            for status_id, status in status_map.items():
                status_age = boundary - timestamp_to_epoch_seconds(status["request_timestamp"])
    ...
    def set_next(self, objects, args):
        ...
        for arg in args:
            ...
            new_list.sort()
            args[arg] = new_list
        d = {"objects": objects, "args": args, "request_time": timestamp_to_epoch_seconds(get_timestamp())}
        self.next[u] = d
        return u
    ...
    def get_next(self, filter_args, lim):
        ...
        if paging_record:
            ...
            headers = {
                "X-TAXII-Date-Added-First": timestamp_to_taxii_json(
                    next_page[0]["__meta"].date_added
                ),
                "X-TAXII-Date-Added-Last": timestamp_to_taxii_json(
    ...
    def _get_objects(
        self, api_root, collection_id, filter_args, limit
    ):
        """
        ...
        if collection:
            ...
            if paging_key:
                ...
                full_filter = MemoryFilter(
                    filter_args, self.interop_requirements_enforced
                )
    ...
    def get_object_manifest(self, api_root, collection_id, filter_args, limit):
        ...
        result = self._get_objects(
            api_root, collection_id, filter_args, limit
        )
            ...
            for new_obj in objs["objects"]:
                ...
                try:
                    ...
                    status_details = generate_status_details(
                        new_obj["id"],
                        timestamp_to_stix_json(new_obj["__meta"].version),
                        message
                    )
                    ...
                    if version:
                        version = timestamp_to_stix_json(version)
    ...
    def delete_object(self, api_root_name, collection_id, obj_id, filter_args):
    ...
    def get_object_versions(self, api_root, collection_id, object_id, filter_args, limit):
        ...
        # We don't expect any version filtering/limiting on this endpoint.
        ...
        if result:
            ...
            versions = [
                timestamp_to_stix_json(obj["__meta"].version)
